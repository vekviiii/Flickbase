...
const getArticleById = async(_id, user) => {
    try {
        if(user.role === 'user')
            {
                throw new ApiError(httpStatus.BAD_REQUEST,'idhar nhi aaneka')
            }
        const article = await Article.findById(_id).populate('category')
        if(!article) throw new ApiError(httpStatus.NOT_FOUND, 'Nhi hain ee wala article')
        return article
...
const updateArticleById = async(_id, body) => {
    try {
        ...
        if(!article) throw new ApiError(httpStatus.NOT_FOUND, 'Nhi hain ee wala article, bhaiya')
        return article
...
const findAllCategories = async(body)=>{
    try {
...
const getUsersArticleById = async(_id)=>{
    try {
        ...
        if(article.status === 'draft') throw new ApiError(httpStatus.BAD_REQUEST, 'not allowed')
        return article
...
const paginateAdminArticles = async(req)=>{
    ...
    try {
        ...
        const limit = await req.body.limit ? req.body.limit : 5
        const options = {
            page: req.body.page,
            limit,
            sort : {_id:"desc"}
